...
class EnhancedWebCrawler:
    ...
    def resolve_redirects(self, url):
        ...
        try:
            ...
            # Parse query parameters from all URLs in the chain
            ...
            for u in all_urls:
                ...
                for param_values in params.values():
                    for value in param_values:
                        matched_kws = self.get_matched_keywords(value)
                        if matched_kws:
                            self.add_result(
                                source_url=url,
                                matched_url=u,
                                ...
                                location_type='redirect_query_param'
                            )
    ...
    def get_matched_keywords(self, text):
        ...
        exact_matches = []
        # Exact keyword matching to reduce false positives
        for kw in self.keywords:
            kw_lower = kw.lower()
            # Check if keyword is a whole word or a significant part of the string
            ...
            if re.search(pattern_encoded, text_lower):
                exact_matches.append(kw)
        # Generic URL patterns for affiliate links
        url_patterns = [
            r'(?:https?://)?(?:www\.)?gowithguide\.com(?:/[^?\s]*)?(?:\?[^?\s]*)?',
            r'utm_source=([^&]+)',
            r'utm_campaign=([^&]+)',
            r'sv1=([^&]+)',
            r'awc=([^&]+)',
        ...
        for pattern in url_patterns:
            ...
            for match in matches:
                if isinstance(match, str):
                    for kw in self.keywords:
                        if kw.lower() in match.lower() and kw not in exact_matches:
                            # Verify context to reduce false positives
                            if 'http' in text_lower or '=' in text_lower or '/' in text_lower:
                                exact_matches.append(kw)
    ...
    def process_url(self, url):
        ...
        # Enhanced script tag processing for dynamic content
        ...
        for script in script_tags:
            if script.string:
                ...
                # Look for JSON-like structures or assignments
                json_patterns = [
...
def main():
    ...
    # Apply Material UI-inspired CSS
    st.markdown("""
        <style>
        body {
